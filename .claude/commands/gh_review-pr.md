# Review GitHub Pull Request

Please analyze the changes in this PR $ARGUMENTS using 'gh pr view $ARGUMENTS --json title,body,state,author,headRefName,baseRefName,number,url' and 'gh pr diff $ARGUMENTS' and focus on identifying critical issues related to:

- Potential bugs or logic errors
- Performance impacts and inefficiencies
- Security concerns (injection vulnerabilities, auth issues, data exposure)
- Breaking changes and backward compatibility with existing code
- Adherence to project's established architecture and design patterns
- Test coverage for new functionality

Specific areas to check:
- Error handling and edge cases
- API contract changes
- Database migrations or schema changes
- Configuration and environment changes
- Dependencies and third-party library updates

If critical issues are found, list them in a few short bullet points. If no critical issues are found, provide a simple approval.
Sign off with a checkbox emoji: ✅ (approved) or ⚠️ (issues found).

Keep your response concise. Only highlight critical issues that must be addressed before merging. Skip detailed style or minor suggestions unless they impact performance, security, or correctness.
